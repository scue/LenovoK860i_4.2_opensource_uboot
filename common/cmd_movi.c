...
int do_movi(cmd_tbl_t * cmdtp, int flag, int argc, char *argv[])
{
	...
	if (attribute == 0x2) {
		...
		/* jeff, stuttart we will burn bl1+bl2+uboot+tzsw together*/
		start_blk = 0;
		blkcnt = MOVI_BOOTLOADER_TOTAL_BLKCNT; /* jeff, bl1+bl2+uboot+tzsw */
#else		/* SMDK partition*/
		start_blk = image[i].start_blk;
		blkcnt = image[i].used_blk;
#endif
		...
		sprintf(run_cmd,"mmc %s %d 0x%lx 0x%lx 0x%lx",
			rw ? "write":"read", dev_num,
			addr, start_blk, blkcnt); //jeff, org 1024 block = 512k, not enough for new tzsw.bin
		run_command(run_cmd, dev_num);
		printf("completed\n");
		return 1;
	}

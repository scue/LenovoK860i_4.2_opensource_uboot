...
void LCD_Initialize(void)
{
    ...
    //MPLL : 880M, change LCD0 src to EPLL 400M
    if(is_exynos4412_rev20()) {
        Outp32(0x1003c234, (Inp32(0x1003c234)&(~(0xf<<0)))|0x7);
        Outp32(0x1003c534, (Inp32(0x1003c534)&(~(0xf<<0)))|0x0);
    } else { //CLK_BUS_DMC_200_400 MPLL 800M:set LCD src MPLL
    ...
    /* set clock*/
    if(is_exynos4412_rev20())
        Outp32(S5PV310_PA_LCD0, 0x40010140);  //EPLL:400M, div 6, CLKVAL : 5, bit[13:6]
    else
        Outp32(S5PV310_PA_LCD0, 0x400102c0);  //MPLL:800M, div 12, CLKVAL : 11, bit[13:6]
  
    /* display on*/
    udelay(5); 
    Outp32(S5PV310_PA_LCD0, (Inp32(S5PV310_PA_LCD0)&(~(0x3<<0)))|0x3); 

}

void FIMD_reset_VIDTCON0()
{
	Outp32(S5PV310_PA_LCD0+0x10, (Inp32(S5PV310_PA_LCD0+0x10)&(~0xffff00))|((0 << 16) | (5 << 8)));//reconfig vfpd and vbpd
}

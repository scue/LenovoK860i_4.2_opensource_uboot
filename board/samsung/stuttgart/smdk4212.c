...
void dram_init_banksize(void)
{
	...
	pop_id = readl(0x10000004) & 0x30;
	printf("E4412, POP type : %dGB\n", (pop_id == 0x10)?2:1);

	nr_dram_banks = CONFIG_NR_DRAM_BANKS;

	if(pop_id == 0x10)
	{
		gd->bd->bi_dram[0].start = PHYS_SDRAM_1_2G;
		gd->bd->bi_dram[0].size = PHYS_SDRAM_1_SIZE_2G;
		gd->bd->bi_dram[1].start = PHYS_SDRAM_2_2G;
		gd->bd->bi_dram[1].size = PHYS_SDRAM_2_SIZE_2G;
		gd->bd->bi_dram[2].start = PHYS_SDRAM_3_2G;
		gd->bd->bi_dram[2].size = PHYS_SDRAM_3_SIZE_2G;
		gd->bd->bi_dram[3].start = PHYS_SDRAM_4_2G;
		gd->bd->bi_dram[3].size = PHYS_SDRAM_4_SIZE_2G;
	}
	else{
